...
public Alignment(Sequence seqone, Sequence seqtwo, SubstitutionGap param)
{
  ...
  // to 0 and S(m, n) is the edit distance. This requirement is enforced in this template.
	...
	//Set m and n depending on sequence A/B's lengths
	M = seqone.getLength(); 
	N = seqtwo.getLength();
	Smat = new int [M + 1][N + 1];
	//System.out.println("Length 1 = " + M + "\n" + "Length 2 = " + N);
	SubstitutionGap sg = new SubstitutionGap();
	
	//Set sequence variables for A/B
	A = seqone.getSeq();
	B = seqtwo.getSeq();
	
	//Algo. for Assignment 3, converted from psuedocode in project spec
	for (int j = N - 1; j >= 0; j--) {
		
		//if (j == M) {
			//Equal to 0
	//	}	
		Smat[M][j] = Smat[M][j + 1] + sg.getgap();
 	}
	...
	//	}
		...
		for (int j = N - 1; j >= 0; j--) {
			...
			Smat[i][j] = Math.min(Smat[i + 1][j + 1] + temp,  Math.min(Smat[i + 1][j] + sg.getgap(), Smat[i][j + 1] + sg.getgap()));
		}
	}
	//System.out.println("Array 1 = " + Arrays.deepToString(Smat));
	//Save S matrix scalar value
	editDistance = Smat[0][0];
	
	//Create top, mid, bot arrays of sufficient length
	top = new char [M + N + 2];
	mid = new char [M + N + 2];
	bot = new char [M + N + 2];
	
	//Run algo. to append OA given by assignment spec
	int i = 0, j = 0, x = 0, count = 0; //x keeps track on index, I couldn't get it to work with [j-1] index, count keeps track of edit numbers during loop
	while (i <= M && j <= N) {
		if (i == M && j == N) {
			break;
		}
		if ((j == N || i < M) && Smat[i][j] == Smat[i + 1][j] + sg.getgap()) {
			//append to OA
			top[x] = A[i + 1];
			mid[x] = '-';
			bot[x] = ' ';
		...
		if (i == M || Smat[i][j] == Smat[i][j + 1] + sg.getgap()) {
			//append to OA
			top[x] = ' ';
			mid[x] = '-';
			bot[x] = B[j + 1];
